...
function SidebarItem({ icon, label, active, onClick, count, color }: { icon: React.ReactNode, label: string, active?: boolean, onClick: () => void, count?: number, color?: string, key?: React.Key }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className={`${active ? (color ? '' : 'text-white dark:text-slate-900') : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'} transition-colors`}>
          {icon}
        </span>
